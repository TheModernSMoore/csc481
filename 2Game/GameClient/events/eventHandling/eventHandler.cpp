...
void BulletHandler::onEvent(Event *e) {
    if (e->getType() == BULLET_SHOOT) {
        ...
        if (object->getObjectType().compare("Character") == 0) {
            bullet = new DeathBox(sf::Vector2f(2, 2), true);
            float y = object_bounds.top;
        } else if (object->getObjectType().compare("Enemy") == 0) {
            bullet = new DeathBox(sf::Vector2f(2, 2), false);
            float y = object_bounds.top + object_bounds.height;
        }
