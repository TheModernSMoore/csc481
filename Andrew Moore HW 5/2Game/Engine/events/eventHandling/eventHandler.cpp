...
void CharCollideHandler::onEvent(Event *e) {
    if (e->getType() == CHARACTER_COLLISION) {
        ...
        if (charc->getOther()->getObjectType().compare("DeathBox") == 0) {
            // Raise death event
            DeathBox *bullet = (DeathBox *)charc->getOther();
            if (!(bullet->player_created)) {
                ObjectManager *objectManager = ObjectManager::get();
                objectManager->removeObject(charc->getOther());
...
// Enemy Collider Handler implementation

EnCollideHandler::EnCollideHandler() {}

void EnCollideHandler::onEvent(Event *e) {
    if (e->getType() == ENEMY_COLLISION) {
        EnemyCollision *enc = (EnemyCollision *) e;
        // Do Event handling
        // get character and object, see what object it is, then throw new events
        if (enc->getOther()->getObjectType().compare("DeathBox") == 0) {
            DeathBox *bullet = (DeathBox *)enc->getOther();
            if (bullet->player_created) {
                ObjectManager *objectManager = ObjectManager::get();
                objectManager->removeObject(enc->getEnemy());
                objectManager->removeObject(enc->getOther());
            }
        } else {
            std::cout << "Collision error" << std::endl; // Should be one of the above types
        }
...
void BulletHandler::onEvent(Event *e) {
    if (e->getType() == BULLET_SHOOT) {
        BulletShoot *bul = (BulletShoot *) e;
        Object *object = bul->getObject();
        sf::Rect<float> object_bounds = object->getGlobalBounds();
        DeathBox *bullet = nullptr;
        float x = object_bounds.left + object_bounds.width/2;
        float y = 0;
        if (object->getObjectType().compare("Character") == 0) {
            bullet = new DeathBox(sf::Vector2f(3, 25), true);
            y = object_bounds.top;
        } else if (object->getObjectType().compare("Enemy") == 0) {
            bullet = new DeathBox(sf::Vector2f(3, 25), false);
            y = object_bounds.top + object_bounds.height;
        }
        bullet->setPosition(x, y);
        ObjectManager::get()->addObject(bullet);
    }
}
...
BulletDeathHandler::BulletDeathHandler() {}

void BulletDeathHandler::onEvent(Event *e) {
    if (e->getType() == BULLET_DEATH) {
        BulletDeath *bulde = (BulletDeath *) e;
        ObjectManager::get()->removeObject(bulde->getBullet());
        delete bulde->getBullet();
    }
}